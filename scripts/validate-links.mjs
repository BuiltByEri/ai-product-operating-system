#!/usr/bin/env node
// Checks local Markdown file links. This skips anchors, external URLs,
// generated files, and semantic link correctness.
...
function validateMarkdown(file) {
  ...
  for (const match of text.matchAll(linkPattern)) {
    ...
    if (!resolved.startsWith(root)) {
      problems.push(`${path.relative(root, file)} links outside repository: ${rawTarget}`);
      continue;
    }
...
if (problems.length) {
  console.error("AAPOS link validation failed:");
  for (const problem of problems) console.error(`- ${problem}`);
  process.exit(1);
}

console.log("AAPOS link validation passed.");
